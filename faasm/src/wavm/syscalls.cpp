...
// 这个函数的作用是为了保证WAVM_DEFINE_INTRINSIC_FUNCTION宏函数能够正常的运行
// WAVM_DEFINE_INTRINSIC_FUNCTION是定义了一个静态全局变量，变量调用了类的构造函数，将会通过构造函数定义的Function添加到一个static的map中
// 正常运行是没有问题的，但是如果将只包含AVM_DEFINE_INTRINSIC_FUNCTION宏的文件，编译为静态链接库，那么此宏将不会执行
// 可以编译为动态链接库，或者是在文件中定义一个函数，并在其他文件中调用，也就是下面的linkHook()
// 原因没有研究过
void linkHook()
{
    chainLink();
